...
/**
 * Allow the new user to sign up with the system.
 * Username automatically generates.
 * 
...
//Class Constructor
	public SignUpWindow() {
		...
		JLabel lblNewLabel = new JLabel("Sign Up",SwingConstants.CENTER);
		...
		textField.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					...
					String username;
					username =user1.getUser_nic().substring(0, 9)+user1.getUser_firstname().substring(0, 3).toLowerCase();
			...
			public void actionPerformed(ActionEvent arg0) {
				try {
					...
					if(!password.equals(password1)) {
						...
						connection=RemoteConnection.remoteConnect();
						user1=connection.getDeserializedUserDetails(); 
						connection.updateRegistration(user1.getUser_nic());
						connection.createSatisficationTableColumn(user1.getUser_nic());
						connection.setUsernameAndPassword(username, password, user1.getUser_nic());
						JOptionPane.showMessageDialog(null,"Account Created Successfully");
						connection.updateWinningProgressValue(10, user1.getUser_nic());
